...
MIN_MONTHS_FOR_FORECAST = MIN_MONTHS_FOR_PROPHET_USER


def _iso_week_start(dates: pd.Series) -> pd.Series:
...
def _evaluate_holdout_month(user_monthly: pd.DataFrame, test_month) -> pd.DataFrame:
...
def expenses_to_weekly(expenses: pd.DataFrame) -> pd.DataFrame:
  ...
  df["week_start"] = _iso_week_start(df["transaction_date"])
  return (
...
def drop_incomplete_current_month(
  ...
  *,
  ...
  cutoff = pd.Timestamp(ref.replace(day=1))
  m = monthly.copy()
...
def calculate_mape_walk_forward(user_monthly: pd.DataFrame) -> tuple[float, pd.DataFrame]:
  ...
  for test_month in months[6:]:
    eval_df = _evaluate_holdout_month(user_monthly, test_month)
    if eval_df.empty:
      continue
...
def calculate_holdout_mape(user_monthly: pd.DataFrame) -> float | None:
  ...
  eval_df = _evaluate_holdout_month(user_monthly, months[-1])
  if eval_df.empty:
    return None
